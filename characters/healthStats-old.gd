...
func _enter_tree():
  ...
  composite.new_method('heal_by', Callable(self, 'heal_by'))
  composite.new_method('overheal_by', Callable(self, 'overheal_by'))
...
func heal_by(change: float):
  var new_hp = (hp + change) if can_overheal else min(max_hp, hp + change)
  composite.set_prop('hp', new_hp)
